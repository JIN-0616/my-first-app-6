...
from django.conf.global_settings import LOGOUT_REDIRECT_URL, LOGIN_URL,\
    AUTHENTICATION_BACKENDS, MEDIA_URL, MEDIA_ROOT
...
#reverse_lazy(): 해당 함수를 호출했을 때 웹서버 실행이 준비되면 urls 파일을 탐색해  url주소를 찾음
#햇갈릴경우 lazy로 통일하는것을추천(error문제)
...
#클라이언트 id
SOCIAL_AUTH_GOOGLE_OAUTH2_KEY = '722075207533-9s90n9use4kqjj8j1vti7kco21vbc67m.apps.googleusercontent.com'
#클라이언트 보안비밀
SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET= 'vfaMsTo7Z0l22i_QGfYssrJm'
...
SECRET_KEY = 'ktd%*cevwbufmcg%038u=f0odekd(mp1g4-i9j@=@0j15$i5^d'
...
TEMPLATES = [
    {
        ...
        'DIRS': [BASE_DIR],# 어느 폴더에 HTML파일을 저장하는지 설정
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
